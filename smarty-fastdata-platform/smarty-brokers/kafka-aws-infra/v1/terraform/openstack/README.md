# kafka on Openstack with Terraform

Provision a kafka cluster with [Terraform](https://www.terraform.io) on
Openstack.

## Status

This will install a kafka cluster on an Openstack Cloud. It should work on
...
to actually install kafka and stand up the cluster.
...
### kafka Nodes
You can create many different kafka topologies by setting the number of
...
|---------|-------------|
|`cluster_name` | All OpenStack resources will use the Terraform variable`cluster_name` (default`example`) in their name to make it easier to track. For example the first compute resource will be named`example-kafka-1`. |
...
|`number_of_k8s_nodes`, `number_of_k8s_nodes_no_floating_ip` | kafka worker nodes. These can be provisioned with or without floating ip addresses. |
...
Terraform can output values that are useful for configure Neutron/Octavia LBaaS or Cinder persistent volume provisioning as part of your kafka deployment:
...
### Set up kubectl
1. [Install kubectl](https://kafka.io/docs/tasks/tools/install-kubectl/) on your workstation
...
```
3. List kafka certificates & keys:
```
ssh [os-user]@[master-ip] sudo ls /etc/kafka/ssl/
```
4. Get `admin`'s certificates and keys:
```
ssh [os-user]@[master-ip] sudo cat /etc/kafka/ssl/admin-[cluster_name]-k8s-master-1-key.pem > admin-key.pem
ssh [os-user]@[master-ip] sudo cat /etc/kafka/ssl/admin-[cluster_name]-k8s-master-1.pem > admin.pem
ssh [os-user]@[master-ip] sudo cat /etc/kafka/ssl/ca.pem > ca.pem
```
5. Configure kubectl:
```ShellSession
...
Try out your new kafka cluster with the [Hello kafka service](https://kafka.io/docs/tasks/access-application-cluster/service-access-application-cluster/).
